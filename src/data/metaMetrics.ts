...
export const metaDays: DayEntry[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { id: 'meta-session-95', dayId: 'Apr 17', label: '#107 metrics-push task-file sync + S95 closeout regen', workCategory: 'Tooling', driver: 'collaborative', operator: 'claude-code', timeMinutes: 300, linesAdded: 533, linesDeleted: 484, note: 'Brainstorm → spec → plan → execute → 2 rounds review → 2 rounds smoke fixes. Task #107 shipped: metrics-push workflow command full rewrite with 6-step flow (gather/classify/prompt/stage/validate/commit-or-rollback), pre-flight idempotency, 3-way status linking, atomic backup/rollback. session-end workflow command gained read-only verify step. How We Work.md got new section. Stage 1 = prompt-and-fill (auto-sync Option A deferred). Classification heuristics validated 5/5 on S94 PRs. Smoke test surfaced + fixed 5 real bugs (regex not multi-line, leaked uncommitted Metrics.ts, pre-git-setup task writes, ambiguous plumbing prompt, done.md wikilink double-add). Dogfood surfaced data integrity fix: 14 historical-backfill task IDs (Phase 2) colliding with modern tasks; renumbered to 9xx (13 pairs on ids 3, 102-114). Generator emits 139 distinct tasks, zero duplicate IDs. No PR for commands (outside repo). PR #188 carries the regen + renumber work: 533/484 in generated-tracker-data.ts only.', contextWindowOrigin: false }, // estimated
    ],
