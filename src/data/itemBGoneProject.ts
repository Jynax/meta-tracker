  ...
  chapters: [
    ...
    },
    {
      ...
      nodes: [
        ...
        {
          id: 'ibg-confidence-tooltips',
          type: 'decision',
          dayId: 'Mar 12',
          category: 'ux-design',
          title: 'Confidence Badges Get Tooltips, Not Label Changes',
          description:
            'Confidence badges show tooltips explaining what each level means, rather than changing label text. Keeps the UI clean while giving users contextual information.',
          chosenPath: 'Tooltips on confidence badges for contextual explanation',
          alternatives: ['Changing badge label text per confidence level'],
        },
        {
          id: 'ibg-stat-card-filters',
          type: 'decision',
          dayId: 'Mar 12',
          category: 'ux-design',
          title: 'Stat Cards as Filter Mechanism',
          description:
            'Stat cards (Safe/Keep/Unknown) are clickable to filter items by verdict. Coverage legend is read-only. One clear interaction point for filtering.',
          chosenPath: 'Clickable stat cards for verdict filtering',
          alternatives: ['Separate filter controls', 'Clickable legend'],
        },
        {
          id: 'ibg-remove-filter-pills',
          type: 'decision',
          dayId: 'Mar 12',
          category: 'ux-design',
          title: 'Remove Category Filter Pills',
          description:
            'Removed the category filter pill bar entirely. Stat cards + search + sort is sufficient. Pills were redundant and cluttered the header.',
          chosenPath: 'Remove pills — stat cards + search + sort is enough',
          alternatives: ['Keep pills alongside stat cards'],
        },
        {
          id: 'ibg-gitignore-data-json',
          type: 'decision',
          dayId: 'Apr 1',
          category: 'process',
          title: 'Gitignore data.json — User State Not Source Code',
          description:
            'Gitignored data.json from the dashboard repo. 7,101 lines of churning WoW scan dumps inflated LOC metrics and bloated the repo. User state shouldn\'t live in source control.',
          chosenPath: 'Gitignore data.json, add sample-data.json for fallback',
          alternatives: ['Keep committing data.json', 'Move to database immediately'],
        },
        {
          id: 'ibg-sample-data-fallback',
          type: 'decision',
          dayId: 'Apr 1',
          category: 'technical',
          title: 'Dashboard Falls Back to sample-data.json',
          description:
            'Dashboard falls back to sample-data.json when data.json is absent. Ensures the live site always shows something, even without a fresh scan.',
          chosenPath: 'sample-data.json as fallback when data.json missing',
          alternatives: ['Show empty state', 'Require data.json to exist'],
        },
        {
          id: 'ibg-r2-scan-storage',
          type: 'decision',
          dayId: 'Apr 5',
          category: 'technical',
          title: 'Cloudflare R2 for Scan Data Storage',
          description:
            'Store scan data in Cloudflare R2 instead of committing to repo. Storage abstracted behind a provider interface for future Supabase migration. R2 is free tier, already on CF account.',
          chosenPath: 'Cloudflare R2 with storage provider abstraction',
          alternatives: ['Cloudflare KV', 'Cloudflare D1', 'Supabase (deferred)'],
        },
      ],
    },
  ],
