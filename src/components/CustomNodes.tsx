...
function ToolBadge({ toolLabel, tool }: { toolLabel?: string; tool?: TreeNodeData['tool'] }) {
  const color = tool === 'chatgpt' ? 'border-emerald-400/60 text-emerald-200' : tool === 'claude' ? 'border-violet-400/60 text-violet-200' : 'border-cyan-400/60 text-cyan-200';
...
export function PhaseNode({ id, data }: NodeProps<InteractiveData>) {
  ...
  return (
    <div className="w-64 rounded-xl border border-sky-400/60 bg-slate-800/90 p-4 text-slate-100 shadow-lg transition hover:brightness-110">
      {!isRoot && <Handle type="target" position={Position.Top} className="!bg-sky-300" />}
      <button className="w-full text-left" onClick={() => data.onToggleExpand?.(id)}>
        ...
        {data.toolLabel && <div className="mt-2"><ToolBadge toolLabel={data.toolLabel} tool={data.tool} /></div>}
      </button>
      <Handle type="source" position={Position.Bottom} className="!bg-sky-300" />
    </div>
  );
}
...
export function DecisionNode({ id, data }: NodeProps<InteractiveData>) {
  return (
    <div className="w-64 rounded-xl border border-emerald-400/70 bg-emerald-950/40 p-3 text-slate-100 transition hover:brightness-110">
      ...
      {data.detailOpen && data.description && <p className="mt-2 border-t border-emerald-300/20 pt-2 text-xs text-slate-200">{data.description}</p>}
...
export function EventNode({ id, data }: NodeProps<InteractiveData>) {
  return (
    <div className="w-56 rounded-xl border border-cyan-400/70 bg-cyan-950/40 p-3 text-slate-100 transition hover:brightness-110">
      ...
      {data.detailOpen && data.description && <p className="mt-2 text-xs text-cyan-100">{data.description}</p>}
    </div>
  );
}
...
export function DeadEndNode({ id, data }: NodeProps<InteractiveData>) {
  return (
    <div className="w-56 rounded-xl border border-rose-400/70 bg-rose-950/40 p-3 text-slate-100 opacity-60 transition hover:brightness-110">
      ...
      {data.detailOpen && data.failureReason && <p className="mt-2 text-xs text-rose-100">{data.failureReason}</p>}
    </div>
  );
}
